...
#include "../game/MoveGeneratorConstants.h"

using namespace torch;

struct NetImpl : public nn::Module {
	nn::Conv1d conv1, conv2, conv3, conv4, convP1, convV1;
	nn::BatchNorm1d bn1, bn2, bn3, bn4, bnP1, bnV1, fcBnP1, fcBnV1;
	 ...
	 */
	NetImpl() :
			conv1(nn::Conv1dOptions(1, 64, 3).stride(1).padding(1)),
			conv2(nn::Conv1dOptions(64, 64, 3).stride(1).padding(1)),
			conv3(nn::Conv1dOptions(64, 64, 3).stride(1).padding(1)),
			conv4(nn::Conv1dOptions(64, 64, 3).stride(1).padding(1)),
			convP1(nn::Conv1dOptions(64, 64, 3).stride(1).padding(1)),
			convV1(nn::Conv1dOptions(64, 64, 3).stride(1).padding(1)),
			fcP1(64 * GAME_STATE_DATA_LENGTH, 512), fcV1(64 * GAME_STATE_DATA_LENGTH, 512),
			bn1(64), bn2(64), bn3(64), bn4(64),
			bnP1(64), bnV1(64), fcBnP1(512), fcBnV1(512), fcP2(512, NUM_MOVES), fcV2(512, 1)
	{
	...
	std::vector<Tensor> forward(Tensor tensor){
		tensor = tensor.view({-1, 1, GAME_STATE_DATA_SIZE[0], GAME_STATE_DATA_SIZE[1], GAME_STATE_DATA_SIZE[2]}); //batchSize by 1 by GAME_STATE_DATA_SIZE

		tensor = relu(bn1(conv1(tensor))); //batchSize by 64 by GAME_STATE_DATA_SIZE
		tensor = relu(bn2(conv2(tensor))); //batchSize by 64 by GAME_STATE_DATA_SIZE
		tensor = relu(bn3(conv3(tensor))); //batchSize by 64 by GAME_STATE_DATA_SIZE
		tensor = relu(bn4(conv4(tensor))); //batchSize by 64 by GAME_STATE_DATA_SIZE

		Tensor probabilities = relu(bnP1(convP1(tensor)));
		...
		Tensor value = relu(bnV1(convV1(tensor)));
